...
const CommunityPage: React.FC = () => {
	...
	const handleConfirmDelete = async () => {
		...
		try {
			if (type === 'post') {
				const res = await fetch(`${API_URL}/posts/${id}`, { method: 'DELETE', headers: headers });
				if (res.ok) {
				...
				const res = await fetch(`${API_URL}/posts/comments/${id}`, { method: 'DELETE', headers: headers });
				if (res.ok) {
					setPosts((prev) => prev.map((p) => {
						if (p.id === parentId && p.comments) {
							return { ...p, commentCount: p.commentCount - 1, comments: p.comments.filter((c) => c.id !== id) };
						}
						return p;
					}));
	...
	const handleSendComment = async (postId: number) => {
		...
		try {
			const body = { content: commentContent, imageUrl: commentImage, stickerUrl: commentSticker, parentId: replyingToCommentId };
			const headers: HeadersInit = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` };
			const res = await fetch(`${API_URL}/posts/${postId}/comments`, { method: 'POST', headers: headers, body: JSON.stringify(body) });
			if (res.ok) {
	...
	const handleCommentLike = async (postId: number, commentId: number) => {
		...
		try {
			await fetch(`${API_URL}/posts/comments/${commentId}/like`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
		} catch (e) {
			console.error(e);
		}
	};
	...
	const submitReport = async () => {
		...
		const url = reportTarget.type === 'post' ? `${API_URL}/posts/${reportTarget.id}/report` : `${API_URL}/posts/comments/${reportTarget.id}/report`;
		try {
	...
	return (
		<div className="comm-layout">
            ...
            <main className="comm-feed">
                ...
                {loading ? (
                    <div style={{textAlign: 'center', padding: 40, color: 'var(--clr-text-secondary)'}}>
                        Đang tải bảng tin...
                    </div>
                ) : sortedPosts.length > 0 ? (
                    sortedPosts.map((post) => (
                        <div key={post.id} className="comm-card" style={{padding: 0, overflow: 'hidden', border: '1px solid var(--clr-border-light)'}}>
                            <PostItem
                                post={post}
                                currentUser={currentUser}
