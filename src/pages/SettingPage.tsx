...
const CURSOR_FILE_NAMES: { [key: string]: string } = {
    'default': 'Alternative.cur', 'pointer': 'link.cur', 'text': 'Escritura a Mano.cur',
    'move': 'movec.cur', 'help': 'ayuda.cur', 'nwse': 'diagonal resize 1.cur',
    'nesw': 'diagonal resize 2.cur', 'ew': 'horizontal.cur',
};
...
const applyCursorStyles = (packId: string) => {
    ...
    Object.entries(CURSOR_FILE_NAMES).forEach(([key, fileName]) => {
        root.style.setProperty(`--cursor-path-${key}`, `url('${selectedPack.basePath}/${fileName}')`);
    });
