...
const PhysicalComicsPage: React.FC = () => {
	...
	const [categoryTitle] = useState('Truyện Tranh Giấy');
	...
	useEffect(() => {
		...
		fetch(`${API_URL}/comics?limit=1000`)
			...
			.then((responseData: any) => {
				const comicsArray = Array.isArray(responseData)
					? responseData
					: responseData.data || [];
				const normalizedComics = comicsArray
					.filter(
						(c: any) =>
							c.isDigital === 0 || c.isDigital === false || c.isDigital === 'false',
					)
					.map((c: any) => ({
						...c,
	...
	const handleSortChange = (newSort: SortState, changedCategory?: keyof SortState | 'reset') => {
		...
		if (changedCategory === 'reset') {
			...
			setSortPriority((prev) => [
				changedCategory,
				...prev.filter((c) => c !== changedCategory),
			]);
		}
	};
		...
		if (filters.genres.length > 0) {
			result = result.filter((c) => {
				...
				const comicGenres = Array.isArray(c.genres)
					...
					: (typeof c.genres === 'string' ? (c.genres as string).split(',') : []).map(
