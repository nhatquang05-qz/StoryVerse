...
const ChapterChat: React.FC<ChapterChatProps> = ({ comicId, chapterId }) => {
	...
	return (
		<div className="chapter-chat-container">
			<div className="chat-room-header">Bàn luận</div>

			<div className="chat-messages-list" ref={chatMessagesListRef}>
				{isLoading && (
					<div
						className="chat-login-prompt"
						style={{ border: 'none', color: 'var(--clr-text-secondary)' }}
					>
						Đang tải bình luận...
					</div>
				)}
			...
			{currentUser ? (
				...
				<div className="chat-login-prompt">Bạn phải đăng nhập để bình luận.</div>
			)}
