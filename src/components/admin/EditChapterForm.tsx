...
import { FiSave, FiTrash2, FiArrowLeft, FiArrowRight, FiPlus, FiRefreshCw, FiDownload } from 'react-icons/fi';
...
const EditChapterForm: React.FC<EditChapterFormProps> = ({
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updateRes = await fetch(`${API_BASE_URL}/comics/${comicId}/chapters/${chapterId}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className="admin-form-container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                >
                    {isSaving ? `Đang xử lý (${uploadProgress.toFixed(0)}%)...` : <><FiSave /> Lưu Toàn Bộ</>}
                </button>
            </div>

            <div className="admin-form" style={{ maxWidth: '100%' }}>
                <div style={{ marginBottom: '20px', padding: '10px', background: '#f5f5f5', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                    ...
                    <div>
                        <input 
                            ...
                            style={{ display: 'none' }} 
                        />
                        <label htmlFor="add-chapter-images" className="mgmt-btn edit" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    ...
                    {images.map((img, index) => (
                        <div key={img.id} style={{ 
                            ...
                            <div style={{ 
                                ...
                                padding: '2px 8px', borderRadius: '4px', fontSize: '12px', zIndex: 2 
                            }}>
                                #{index + 1}
                            </div>
                            ...
                            <button 
                                ...
                                style={{ 
                                    ...
                                    cursor: 'pointer', zIndex: 2, color: '#333'
                                }}
                                title="Tải ảnh này"
                            >
                                <FiDownload size={14} />
                            ...
                            <div style={{ height: '250px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#333', borderRadius: '4px', marginBottom: '10px', overflow: 'hidden' }}>
                                <img 
                                    src={img.src} 
                                    alt={`Page ${index + 1}`} 
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px' }}>
                                    <label className="mgmt-btn edit" style={{ padding: '5px', justifyContent: 'center', fontSize: '12px', cursor: 'pointer' }}>
                                        <FiRefreshCw /> Thay
                                    ...
                                    <button 
                                        ...
                                        style={{ padding: '5px', justifyContent: 'center', fontSize: '12px' }}
                                    >
                                        <FiTrash2 /> Xóa
                                    </button>
                                </div>
