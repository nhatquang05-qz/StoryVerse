...
const DashboardView: React.FC = () => {
	...
	const handleBanUser = async () => {
		...
		if (
			!window.confirm(
				`CẢNH BÁO: Bạn sắp BAN tài khoản "${selectedReport.reportedUserName}" và XOÁ nội dung này. Hành động này không thể hoàn tác. Tiếp tục?`,
			)
		)
			return;
	...
	const getTypeBadge = (type: string) => {
		...
		if (['nạp', 'deposit', 'recharge', 'nap_xu'].some((k) => typeLower.includes(k))) {
			...
			['mua', 'purchase', 'buy', 'unlock', 'mua_truyen'].some((k) => typeLower.includes(k))
		) {
	...
	const polarData = {
		labels:
			data.revenueSource.labels.length > 0 ? data.revenueSource.labels : ['Chưa có dữ liệu'],
		datasets: [
			{
				label: 'Nguồn thu',
		...
		<div className="dashboard-container">
					...
					<div className="stat-header">
						<div>
							...
							<h3 className="stat-value">
								{data.stats.users.toLocaleString('vi-VN')}
							</h3>
						</div>
				...
				<div className="stat-card border-amber">
					<div className="stat-header">
						<div>
							...
							<h3 className="stat-value">
								{data.stats.orders.toLocaleString('vi-VN')}
							</h3>
						</div>
				...
				<div className="stat-card border-blue">
					<div className="stat-header">
						<div>
							...
							<h3 className="stat-value">
								{data.stats.comics.toLocaleString('vi-VN')}
							</h3>
						</div>
			...
			<div className="system-health-grid">
				<div className="health-card pending-tasks">
					<h3 className="card-title-sm" style={{ color: 'red' }}>
						<FiAlertCircle /> Cần Xử Lý Ngay
					</h3>
					<ul className="task-list">
						<li>
							<span
								...
								style={{
									backgroundColor: postReports.length > 0 ? '#ef4444' : '#10b981',
								}}
							>
								{postReports.length}
							</span>
						...
						<li>
							<span
								...
								style={{
									backgroundColor:
										commentReports.length > 0 ? '#f59e0b' : '#10b981',
								}}
							>
			...
			<div className="chart-triple-view">
				...
				<div className="sub-chart-card">
					<div className="chart-header">
						<h3 className="chart-title">Phân Bố Thể Loại</h3>
					</div>
					<div className="sub-chart-wrapper">
				...
				<div className="sub-chart-card">
					<div className="chart-header">
						<h3 className="chart-title">Nguồn Doanh Thu</h3>
					</div>
					<div className="sub-chart-wrapper">
			...
			<div className="transactions-table-container" style={{ marginBottom: '2rem' }}>
				<div className="table-header">
					<h3 className="chart-title flex-align">
						<FiTrendingUp /> Top Truyện Bán Chạy Nhất
					</h3>
				</div>
				<div className="table-responsive">
					<table className="custom-table">
								...
								data.topComics.map((comic: any, index: number) => (
									<tr key={comic.id || index}>
										...
										<td className="comic-revenue-cell">
											{formatCurrency(comic.totalRevenue)}
										</td>
									</tr>
								))
							) : (
					...
					<table className="custom-table">
						...
						<tbody>
							{data.transactions && data.transactions.length > 0 ? (
								...
								<tr>
									<td colSpan={6} className="empty-state-cell-large">
