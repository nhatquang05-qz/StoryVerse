...
const EditComicForm: React.FC<EditComicFormProps> = ({ comic, allGenres, onCancel, onSuccess }) => {
	...
	const handleDeleteComic = async () => {
		if (
			!window.confirm(
				`Bạn có chắc chắn muốn XÓA vĩnh viễn truyện "${comic.title}" không? Hành động này không thể hoàn tác.`,
			)
		) {
			return;
		}
	...
	if (isLoading) {
		return (
			<div className="admin-form-container">
				<p style={{ padding: '2rem', textAlign: 'center' }}>Đang tải chi tiết truyện...</p>
			</div>
		);
	}
	...
	return (
		<div className="admin-form-container">
			...
			<form onSubmit={handleUpdateComic} className="admin-form">
				<h2>
					Sửa Truyện: {formData.title} ({formData.isDigital ? 'Online' : 'In Ấn'})
				</h2>
				...
				<div className="form-group">
					<label>
						Ảnh bìa (URL) <span style={{ color: 'red' }}>*</span>:
					</label>
				...
				<div
					style={{
						display: 'grid',
						gridTemplateColumns: formData.isDigital ? '1fr' : '1fr 1fr',
						gap: '1rem',
					}}
				>
					<div className="form-group">
