...
const PackManagement: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'packs' | 'vouchers'>('packs');

    const [packs, setPacks] = useState<any[]>([]);
    const [isEditing, setIsEditing] = useState<any>(null);
    const [packFormData, setPackFormData] = useState({ coins: 0, price: 0, bonus: 0, isActive: true });
    ...
    const handleVoucherSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const url = isEditingVoucher.id ? `${API_BASE_URL}/vouchers/${isEditingVoucher.id}` : `${API_BASE_URL}/vouchers`;
            ...
            if (res.ok) {
                showNotification(isEditingVoucher.id ? 'Cập nhật voucher thành công' : 'Thêm voucher thành công', 'success');
    ...
    const openEditPack = (pack: any) => {
        setIsEditing(pack);
        setPackFormData({ coins: pack.coins, price: pack.price, bonus: pack.bonus, isActive: pack.isActive === 1 });
    };
                            ...
                            <form onSubmit={handlePackSubmit} className="pack-form">
                                <div className="form-group">
                                    ...
                                    <input 
                                        ...
                                        onChange={e => setPackFormData({...packFormData, coins: Number(e.target.value)})} 
                                        required 
                                        placeholder="VD: 1000"
                                    />
                                </div>
                                ...
                                <div className="form-group">
                                    ...
                                    <input 
                                        ...
                                        onChange={e => setPackFormData({...packFormData, bonus: Number(e.target.value)})} 
                                        placeholder="VD: 100"
                                    />
                                </div>
                                    ...
                                    {packs.map(pack => (
                                        <tr key={pack.id}>
                                            ...
                                            <td className="action-buttons">
                                                <button className="mgmt-btn edit" onClick={() => openEditPack(pack)}><FiEdit /></button>
                                                <button className="mgmt-btn delete" onClick={() => handleDeletePack(pack.id)}><FiTrash2 /></button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                <div className="tab-content">
                    ...
                    {isEditingVoucher ? (
                        <div className="edit-form-wrapper wide-form">
                            <h3 className="form-title">{isEditingVoucher.id ? 'Sửa Voucher' : 'Tạo Voucher Mới'}</h3>
                            <form onSubmit={handleVoucherSubmit} className="pack-form voucher-form-grid">
                                ...
                                <div className="form-group">
                                    ...
                                    <input 
                                        ...
                                        onChange={e => setVoucherFormData({...voucherFormData, maxDiscountAmount: Number(e.target.value)})} 
                                ...
                                <div className="form-group full-width">
                                    <label className="checkbox-group">
                                        ...
                                        />
                                        <span className="checkbox-label">Kích hoạt voucher này</span>
                                    </label>
                                </div>
                                
                                <div className="form-actions full-width">
                                    <button type="submit" className="btn-action save-btn">Lưu Voucher</button>
                                    <button type="button" className="btn-action cancel-btn" onClick={() => setIsEditingVoucher(null)}>Hủy</button>
                                </div>
                            </form>
                        </div>
                    ) : (
                            ...
                            <table className="admin-user-table">
                                ...
                                <tbody>
                                    {vouchers.map(voucher => (
                                        <tr key={voucher.id}>
                                            <td style={{fontWeight:'bold', color:'#3b82f6'}}>{voucher.code}</td>
                                            <td>{voucher.discountType === 'PERCENT' ? 'Phần trăm' : 'Cố định'}</td>
                                            <td style={{color:'#eab308', fontWeight:'bold'}}>
                                                {voucher.discountType === 'PERCENT' ? `${voucher.discountValue}%` : `${Number(voucher.discountValue).toLocaleString('vi-VN')}đ`}
                                            </td>
                                            ...
                                            <td>
                                                {voucher.isActive ? 
                                                    <span className="status-tag active"><FiCheck /></span> : 
                                                    <span className="status-tag banned"><FiX /></span>
                                                }
                                            </td>
                                            <td className="action-buttons">
                                                <button className="mgmt-btn edit" onClick={() => openEditVoucher(voucher)}><FiEdit /></button>
                                                <button className="mgmt-btn delete" onClick={() => handleDeleteVoucher(voucher.id)}><FiTrash2 /></button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
