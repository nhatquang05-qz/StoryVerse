...
const OrderManagement: React.FC = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/orders/admin/all?page=${page}&limit=10`, config);
  ...
  const handleStatusUpdate = async (orderId: number, newStatus: string) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/orders/admin/${orderId}/status`, { status: newStatus }, config);
  ...
  const getStatusBadge = (status: string) => {
      switch(status) {
          ...
          case 'PAID': return <span className="badge badge-info">Đã thanh toán</span>;
          case 'PENDING': return <span className="badge badge-warning">Chờ thanh toán</span>;
        ...
        <table className="order-table">
            ...
            <tbody>
                {orders.map(order => (
                    <tr key={order.id}>
                        <td>
                            <span className={order.transactionCode ? "trans-code" : "trans-code-placeholder"}>
                        ...
                        <td className="total-amount">{Number(order.totalAmount).toLocaleString()}đ</td>
                        <td>
                            <span className={`pay-badge ${order.paymentMethod === 'COD' ? 'pay-badge-cod' : 'pay-badge-online'}`}>
                                {order.paymentMethod}
                            </span>
                        </td>
                        ...
                        <td>
                            <div className="action-buttons">
                                <button onClick={() => viewDetails(order)} className="btn-icon btn-view" title="Xem chi tiết">
                                    <FaEye />
                                </button>
          ...
          <div className="modal-overlay">
              <div className="modal-content">
                  <div className="modal-header">
                      <h3>Chi tiết đơn: {selectedOrder.transactionCode || `#${selectedOrder.id}`}</h3>
                      <button onClick={() => setShowModal(false)} className="close-modal">×</button>
                  </div>
                  <div className="modal-body">
                      <div className="order-info-grid">
                          <p><strong>Mã giao dịch:</strong> {selectedOrder.transactionCode || 'Chưa có'}</p>
                      ...
                      <ul className="item-list">
                          {orderItems.map((item: any, idx) => (
                              <li key={idx} className="order-item-row">
                                  <img src={item.coverImageUrl} alt="" className="item-thumb"/>
                                  <div className="item-details">
                                      <span className="item-title">{item.title}</span>
                                      <span className="item-qty">Số lượng: {item.quantity}</span>
                                      <span className="item-price">{Number(item.price).toLocaleString()}đ</span>
                                  </div>
                              </li>
                          ))}
                      </ul>
                      <div className="modal-total">
                          Tổng cộng: <span>{Number(selectedOrder.totalAmount).toLocaleString()}đ</span>
                      </div>
                  </div>
                  <div className="modal-footer">
                      {selectedOrder.paymentMethod === 'COD' && selectedOrder.status !== 'COMPLETED' && selectedOrder.status !== 'CANCELLED' && (
                          <button 
                              onClick={() => {
                      ...
                      <button onClick={() => setShowModal(false)} className="btn-close">Đóng</button>
                  </div>
              </div>
          </div>
      )}
