	...
	return (
		<div className="comments-section">
				...
				{rootComments.map((comment) => (
					<div key={comment.id} className="comment-thread">
						<CommentItem
							...
							onLike={props.onLikeComment}
							onReply={setReplyingToCommentId}
							onDelete={props.onDeleteComment}
							onReport={props.onReportComment}
			...
			<div className="comment-input-area sticky-input">
				{replyingToCommentId && (
					<div
						className="reply-input-indicator"
						style={{ position: 'absolute', top: '-35px', left: 40, right: 0 }}
					>
						<span>
							Đang trả lời <b>{replyUser}</b>
						</span>
				...
				<div className="comment-input-wrapper">
					<input
						...
						placeholder={
							replyingToCommentId ? 'Viết câu trả lời...' : 'Viết bình luận...'
						}
					...
					<div className="comment-actions-right">
						...
						<button
							className="mini-icon-btn sticker-toggle-btn"
							onClick={(e) => {
								e.stopPropagation();
								props.setShowStickerPicker(!props.showStickerPicker);
							}}
						>
							<FaSmile />
						</button>
						<button
							...
							onClick={() => props.onSend(post.id)}
							disabled={
								(!props.commentContent.trim() &&
									!props.commentImage &&
									!props.commentSticker) ||
								props.isUploading
							}
						>
							<FaPaperPlane />
				...
				{props.showStickerPicker && (
					<div className="sticker-popover">
						<StickerPicker
							onStickerSelect={(s) => {
								props.setCommentSticker(s.url);
								props.setShowStickerPicker(false);
							}}
							onClose={() => props.setShowStickerPicker(false)}
						/>
					</div>
				)}
			</div>
			...
			{props.isUploading ? (
				...
				(props.commentImage || props.commentSticker) && (
					<div className="comment-img-preview-box">
						<img
							src={props.commentImage || props.commentSticker || ''}
							alt="preview"
							style={
								props.commentSticker
						...
						<button
							onClick={() => {
								props.setCommentImage(null);
								props.setCommentSticker(null);
							}}
