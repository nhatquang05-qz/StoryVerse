...
interface CartContextType {
  cartItems: CartItem[];
  addToCart: (comic: ComicSummary | any, quantity: number, startElementRect: DOMRect | null) => void;
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addToCart = async (comic: ComicSummary | any, quantity: number, startElementRect: DOMRect | null) => {
    if (comic.isDigital) {
  ...
  const checkout = async (): Promise<any> => {
    ...
    try {
      const response = await fetch(`${API_URL}/orders`, {
        ...
        body: JSON.stringify({
          items: cartItems.filter(item => !item.isDigital).map(item => ({
            comicId: item.id,
