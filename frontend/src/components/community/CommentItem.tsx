			...
			<div className="comment-content-block">
				...
				>
					<div className="comment-bubble">
						...
						{comment.stickerUrl && (
							<img src={comment.stickerUrl} className="sticker-img" alt="sticker" />
						)}
					</div>
					<div className="comment-menu-trigger" style={{ position: 'relative' }}>
						<button
							className="btn-options small"
							onClick={(e) => {
								e.stopPropagation();
								setActiveMenuId(
									showMenu ? null : { id: comment.id, type: 'comment' },
								);
							}}
						>
							<FaEllipsisH />
				...
				<div className="comment-actions-text">
					...
					>
						...
						{comment.likeCount > 0 && (
							<span className="like-count-badge"> ({comment.likeCount})</span>
						)}
					</span>
					<span className="action-text" onClick={() => onReply(comment.id)}>
						Phản hồi
					</span>
