...
const ProductCard: React.FC<ProductCardProps> = ({ comic, isCarousel = false }) => {
	...
	return (
		<div className={`product-card ${isCarousel ? 'carousel-item' : ''}`}>
			...
			{hasFlashSale && <span className="fs-badge-right">-{discountPercent}%</span>}

			<Link to={`/comic/${comicData.id}`} className="card-image-container">
				<img
					ref={imgRef}
					src={comicData.coverImageUrl}
					alt={comicData.title}
			...
			<div className="card-info">
				...
				<div className="card-rating-section">
					...
					{totalReviews > 0 && (
						<span className="card-view-count" style={{ marginLeft: '0.5rem' }}>
							({totalReviews})
						</span>
					)}
				</div>
				...
				{comicData.isDigital === 1 && (
					<div className="card-view-count-section">
						<span className="card-view-count">
							{formatViewCount(comicData.viewCount)}
						</span>
					</div>
				)}
					...
					<div className="card-price-row">
						{hasFlashSale ? (
							<div className="price-container-fs">
								<span className="card-price fs-price">
									{formatPrice(comicData.flashSalePrice)}
								</span>
								<span className="card-original-price">
									{formatPrice(comicData.price)}
								</span>
							</div>
						) : (
